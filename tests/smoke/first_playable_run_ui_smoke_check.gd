...
func _initialize() -> void:
	...
	_assert_true(hud.get_node_or_null("KillCounter") != null, "HUD must create top-left kill counter", failures)
	...
	_assert_true(hud_text.contains("Kills 3"), "HUD must show current kill count in the top-left counter", failures)
