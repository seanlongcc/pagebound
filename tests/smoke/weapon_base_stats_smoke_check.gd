...
func _initialize() -> void:
	...
	_assert_true(is_equal_approx(star_cooldown, 0.9), "Star Sticker Swarm base cooldown must start at tuned base 0.9s", failures)
	...
	_assert_true(is_equal_approx(manager.debug_cooldown_seconds(), 0.95), "Waxlight base cooldown must start at tuned AoE base 0.95s", failures)
	...
	var first_upgrade: Dictionary = upgrade_state.apply_choice(&"weapon_upgrade_star_sticker_swarm")
	manager.refresh_runtime_modifiers()
	_assert_true(not first_upgrade.is_empty(), "First Star Sticker upgrade must apply", failures)
	_assert_true(upgrade_state.weapon_level(&"star_sticker_swarm") == 2, "Star progress must increment to level 2 after first upgrade", failures)
	_assert_true(is_equal_approx(manager.debug_weapon_damage(&"star_sticker_swarm"), 110.0), "First +10% Star damage upgrade must produce 100 * 1.10 = 110", failures)
	...
	var second_upgrade: Dictionary = upgrade_state.apply_choice(&"weapon_upgrade_star_sticker_swarm")
	manager.refresh_runtime_modifiers()
	_assert_true(not second_upgrade.is_empty(), "Second Star Sticker upgrade must apply", failures)
	...
	_assert_true(is_equal_approx(manager.debug_weapon_damage(&"star_sticker_swarm"), 110.0), "Second non-damage Star upgrade must not add free damage", failures)
	_assert_true(manager.debug_weapon_range_meters(&"star_sticker_swarm") > star_range, "Second Star upgrade must increase node/orbit range", failures)
