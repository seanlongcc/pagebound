...
Use `request_user_input` to surface any open questions on accessibility tier:
...
Use `request_user_input` to step through items in groups of 3-4, not all at once.
...
When delegating to another agent via the `spawn_agent` tool:
