...
Before generating the final verdict, spawn all four directors as **parallel subagents** via `spawn_agent` using the parallel gate protocol from `.codex/docs/director-gates.md`. Issue all four `spawn_agent` calls simultaneously — do not wait for one before starting the next.
...
- **No accessibility requirements doc?** → Use `request_user_input` to offer to create it now:
  ...
  - If "Create it now": use a second `request_user_input` to ask for the tier:
