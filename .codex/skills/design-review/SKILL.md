...
actual `spawn_agent` calls. A simulated review is not a specialist review.**

Issue all `spawn_agent` calls simultaneously. Do NOT spawn one at a time.
...
Work through all blocking items, asking for design decisions only where you cannot resolve the issue from the GDD and existing docs alone. Group all design-decision questions into a single multi-tab `request_user_input` before making any edits — do not interrupt mid-revision for each blocker individually.
