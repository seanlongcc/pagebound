...
user-invocable: true
allowed-tools: exec_command, apply_patch, spawn_agent, request_user_input, update_plan
---
...
Delegate in parallel — issue all three `spawn_agent` calls simultaneously before waiting for any result:
...
sub-agents spawned via `spawn_agent`. Each sub-agent enforces the "May I write to [path]?"
