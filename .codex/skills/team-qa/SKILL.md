...
After each FAIL result: use `request_user_input` to collect the failure description, then spawn `qa-tester` via `spawn_agent` to write a formal bug report in `production/qa/bugs/`.
