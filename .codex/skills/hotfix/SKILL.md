...
user-invocable: true
allowed-tools: exec_command, apply_patch, spawn_agent
---
...
Use the `spawn_agent` tool to request sign-off in parallel:
...
After approvals, determine the QA scope required before deploying the hotfix. Spawn `qa-lead` via `spawn_agent` with:
