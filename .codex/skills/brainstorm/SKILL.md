...
Bundle these into a single multi-tab `request_user_input` with these exact tab labels:
...
**After pillars and anti-pillars are agreed, spawn BOTH `creative-director` AND `art-director` via `spawn_agent` in parallel before moving to Phase 5. Issue both `spawn_agent` calls simultaneously — do not wait for one before starting the other.**
...
**After identifying biggest technical risks, spawn `technical-director` via `spawn_agent` using gate TD-FEASIBILITY (`.codex/docs/director-gates.md`) before scope tiers are defined.**
...
**After scope tiers are defined, spawn `producer` via `spawn_agent` using gate PR-SCOPE (`.codex/docs/director-gates.md`).**
