...
- If it exists: read it, find the first context (system/level/character) with any asset at status "Needed" but no spec file written yet, and use `request_user_input`:
...
Spawn specialist agents based on review mode. **Issue all `spawn_agent` calls simultaneously — do not wait for one before starting the next.**

### Full mode — spawn in parallel:

**`art-director`** via `spawn_agent`:
...
**`technical-artist`** via `spawn_agent`:
