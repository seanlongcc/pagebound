...
```
Spawn `creative-director` via `spawn_agent` using gate **CD-PILLARS** from
...
Spawn `[agent-name]` via `spawn_agent`:
...
Spawn all [N] agents simultaneously via `spawn_agent` — issue all `spawn_agent` calls before
...
Agent: `creative-director` | Model tier: GPT-5.5 | Domain: Vision, pillars, player experience
...
Agent: `technical-director` | Model tier: GPT-5.5 | Domain: Architecture, engine risk, performance
...
Agent: `producer` | Model tier: GPT-5.5 | Domain: Scope, timeline, dependencies, production risk
...
Agent: `art-director` | Model tier: GPT-5.3 Codex | Domain: Visual identity, art bible, visual production readiness
...
specialist's feasibility sign-off is needed. Tier 2 leads use GPT-5.3 Codex (default).
...
```
Spawn in parallel (issue all `spawn_agent` calls before waiting for any result):
