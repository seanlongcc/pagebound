...
|------|-------|-------------|
| **GPT-5.4 Mini** | `gpt-5.4-mini` | Read-only status checks, formatting, simple lookups — no creative judgment needed |
| **GPT-5.3 Codex** | `gpt-5.3-codex` | Implementation, design authoring, analysis of individual systems — default for most work |
| **GPT-5.5** | `gpt-5.5` | Multi-document synthesis, high-stakes phase gate verdicts, cross-system holistic review |
...
All other skills default to GPT-5.3 Codex. When creating new skills, assign GPT-5.4 Mini if the
skill only reads and formats; assign GPT-5.5 if it must synthesize 5+ documents with
high-stakes output; otherwise leave unset (GPT-5.3 Codex).
