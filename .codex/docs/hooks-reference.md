...
| ---- | ----- | ------- | ------ |
| `validate-commit.sh` | PreToolUse (`exec_command`) | `git commit` commands | Validates design doc sections, JSON data files, hardcoded values, TODO format |
| `validate-push.sh` | PreToolUse (`exec_command`) | `git push` commands | Warns on pushes to protected branches (develop/main) |
| `validate-assets.sh` | PostToolUse (`apply_patch`) | Asset file changes | Checks naming conventions and JSON validity for files in `assets/` |
...
| `validate-skill-change.sh` | PostToolUse (`apply_patch`) | Skill file changes | Advises running `/skill-test` after any `.codex/skills/` file is written or edited |
