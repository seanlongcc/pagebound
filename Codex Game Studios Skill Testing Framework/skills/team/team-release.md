...
**Expected behavior:**
1. Phase 1: Spawns `producer` via `spawn_agent`; confirms all milestone acceptance criteria met; identifies any deferred scope; produces release authorization; presents to user; request_user_input: user approves before Phase 2
2. Phase 2: Spawns `release-manager` via `spawn_agent`; cuts release branch from agreed commit; bumps version numbers; invokes `/release-checklist`; freezes branch; output: branch name and checklist; request_user_input: user approves before Phase 3
3. Phase 3 (parallel): Issues `spawn_agent` calls simultaneously for `qa-lead` (regression suite, critical path sign-off) and `devops-engineer` (build artifacts, CI verification); security-engineer is NOT spawned (no online features); network-programmer is NOT spawned (no multiplayer); both complete successfully
...
5. Phase 5: Spawns `producer` via `spawn_agent`; collects sign-offs from qa-lead, release-manager, devops-engineer; no open blocking issues; producer declares GO; request_user_input: user sees GO decision and confirms deployment
...
**Assertions:**
- [ ] Phase 3 qa-lead and devops-engineer `spawn_agent` calls are issued simultaneously, not sequentially
...
5. Phase 5: Spawns `producer` via `spawn_agent`; producer receives qa-lead's NOT PASSED verdict; no S1 sign-off available; producer declares NO-GO with rationale: "S1 bug [ID] is open and unresolved. Releasing is not safe."
...
2. Phase 3 (parallel): Orchestrator detects that the game has online/multiplayer features and player data; issues `spawn_agent` calls simultaneously for `qa-lead`, `devops-engineer`, AND `security-engineer`; also spawns `network-programmer` for netcode stability sign-off
...
- [ ] All four Phase 3 `spawn_agent` calls (qa-lead, devops-engineer, security-engineer, network-programmer) are issued simultaneously
...
- [ ] Phase 3 agents are always issued as parallel `spawn_agent` calls — qa-lead and devops-engineer are never sequential
