...
| **T2 — Parallel where independent** | Agents whose inputs don't depend on each other are spawned in parallel (single message, multiple `spawn_agent` calls) |
...
| **D4 — GPT-5.5 model tier** | Agent is assigned GPT-5.5 model per coordination-rules.md |
...
| **L3 — GPT-5.3 Codex model tier** | Agent is assigned GPT-5.3 Codex model (default) per coordination-rules.md |
