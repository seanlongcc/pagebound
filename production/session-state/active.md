...
- `src/pickups/`
- `src/weapons/mvp_weapon_effects.gd`
...
- `tests/smoke/prototype_weapon_pool_gdd_guard_smoke_check.gd`
- `tests/smoke/passive_tag_modifier_smoke_check.gd`
- `tests/smoke/weapon_range_smoke_check.gd`
...
Corrective MVP tuning pass is now in place with primitive placeholders only. Design source consulted: `PAGEBOUND_CODEX_GDD_v1_5.md` Chapters 11 and 12, plus focused weapons/passives/drafts GDDs. Next practical work: extract `src/runtime/first_playable_runtime.gd` below the local 800-line guardrail, continue moving prototype runtime tuning into focused `.tres` content, add deeper documented weapon/passive behavior, add Page Events/boss timing beyond the prototype 60s Color Well trigger, and replace primitive placeholders only after asset provenance is recorded.
...
- Opening enemy pacing uses only `inkling_chaser` until the 60s pressure band. `paper_scrap_swarmer` joins after 60s. Early spawn pressure uses the corrective mild bump: opening target budget 8, opening interval 1.45s, first-pressure target 16 at 1.10s, then 26 at 0.88s and 42 at 0.70s.
...
- Additional documented primitive weapon-pick options: `dreamsap_glob` drops a sticky Dreamsap puddle/snare placeholder, and `color_bloom` creates a Color Bloom burst/growing-zone placeholder. Non-GDD `paper_plane_dart` and `margin_spark_ring` were removed.
...
- Prototype upgrade steps are chunky/readable and one-stat-per-card: Waxlight damage +2, Waxlight cooldown -0.25s, Waxlight active duration +1s, Waxlight unactivated mark cap +3, player max HP +20, Star Sticker damage +2 is separate from Star Sticker count +1, per-weapon range cards change only max targeting range, and Candle Spark advances in 15% tagged-damage steps.
...
- Large-file note: `src/runtime/first_playable_runtime.gd` remains over the 800-line guardrail. This pass only adjusted display-name wiring there; feature behavior lives in focused runtime helpers (`AutoWeaponManager`, `MvpWeaponEffects`, `PagecraftManager`, `RunDraftController`, `RunUpgradeState`, `PageEventController`, `PageEventAnnouncement`). Extraction below the local guardrail remains tracked in `pagebound-hup`.
...
- `Godot_v4.6.2-stable_win64_console.exe --headless --path . --script tests/smoke/prototype_weapon_pool_gdd_guard_smoke_check.gd` passed.
- `Godot_v4.6.2-stable_win64_console.exe --headless --path . --script tests/smoke/passive_tag_modifier_smoke_check.gd` passed.
- `Godot_v4.6.2-stable_win64_console.exe --headless --path . --script tests/smoke/weapon_range_smoke_check.gd` passed.
...
- First playable behavior: `Main.tscn` boots to a start menu, runtime creates a primitive player, input actions, widened top-down camera follow, runtime event bus, damage model, pooled damage numbers, a scene-owned run director with mild-bump early pressure time bands, two placeholder enemy families (`inkling_chaser` and delayed `paper_scrap_swarmer`), Waxlight Comet, draft-acquired Star Sticker Swarm, and documented Dreamsap Glob / Color Bloom weapon-pick options, documented Candle Spark passive, visible runtime stats HUD, collectible Color Mote XP drops with 3m close-range magnet pull, run XP thresholds, level-up event flow, rarity-weighted exactly-3 vertical draft cards with rarity borders, weapon-pick levels, runtime upgrades for Waxlight damage, Waxlight duration, Waxlight unactivated mark cap, Waxlight cooldown, player max HP, per-weapon range, separate Star Sticker damage/count upgrades, and Candle Spark Firelight/Waxlight-tagged damage, finite 22x14 page bounds clamp, finite spawn bounds, visible Pagecraft marks, dash activation into finite active Waxlight zones, contact-only active Waxlight damage through DamageModel, activated mark decay, finite inactive mark expiry, primitive finite Waxlight dash pulse visuals, prototype 60s Fill the Color Well event/objective with temporary announcement, and 5:00 victory summary. XP awards only after pickup collection. Player death opens a `Run Over` death screen with Retry/Main Menu, pauses/stops gameplay, blocks dead-body XP collection, blocks post-death upgrades/healing, and Retry snaps/rebinds the camera to the new player. Dead enemies visibly despawn, stop physics, disable collision, and stop being targetable. `move_up`/W moves toward negative Z/page top.
- Current implementation count: 35 GDScript source files, 30 smoke checks, 7 ADRs. No third-party gameplay/art assets imported.
