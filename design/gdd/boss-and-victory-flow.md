...
`Boss and Victory Flow` defines 30:00 boss/finale start, boss actor lifecycle, phase handoff, victory trigger, post-boss reward flow, failure/death handoff, and run-complete events. It owns the run climax and resolution. It does not own normal enemy spawning, base damage math, player controller, or permanent meta progression spending.
...
1. Boss/finale starts at 30:00 in normal MVP runs.
2. Strong normal runs should reach run level 50 and have a core-complete build by boss start.
3. Boss spawns from chapter-specific dramatic anchor.
4. Normal Page Events stop at boss start; endless Page Events can continue every 5 minutes after boss start, beginning at 35:00.
...
8. Boss pressure can escalate after it starts, but victory remains possible.
...
|---|---|---|
| `WaitingForFinale` | Run is before 30:00. | `BossWarning`, `RunFailed` |
...
| Run Director and Finite Spawning | Upstream | Starts boss at 30:00 and stops normal event cadence. |
...
`boss_start_ready = run_time_seconds >= 1800`

`boss_pressure_ready = run_time_seconds >= 1800 and boss_alive`
...
- Boss spawns before or after 30:00 in normal MVP timeline.
...
- If player kills boss quickly after spawn, victory starts immediately.
...
- If draft UI is open at 30:00, boss warning queues until draft resolves or run timer pauses with draft.
...
| `pressure_start_seconds` | `1800` | `1800+` | Boss pressure begins at spawn and escalates after start. |
...
- Debug overlay shows boss state, phase, HP, pressure timer, and victory/failure flags.

## Acceptance Criteria

- Boss starts at 30:00 and normal Page Events stop.
